# scripts/ablation_ssm_encoder.py
...
# If build_model already supports "ssm", keep it. Otherwise, you can set this to "transformer".
ENCODER_FOR_SSM = "ssm"

CFG = dict(activation="tanh", optimizer="sgd",
           neg_per_pos=1, walks=16, alpha=0.4, walk_L=6,
           epochs=50, batch=32, lr=1e-2)

OUT = Path("results/ablations/ssm_encoder.csv")
...
def run_once(csv_path, ds_name, split, seed):
    cmd = [
        ...
        "--encoder", ENCODER_FOR_SSM, "--activation", CFG["activation"], "--optimizer", CFG["optimizer"],
        "--neg_per_pos", str(CFG["neg_per_pos"]), "--walks", str(CFG["walks"]),
        "--alpha", str(CFG["alpha"]), "--walk_L", str(CFG["walk_L"]),
        "--no_node2vec",                 # replace Node2Vec with temporal encoder
